...
![Environment tuple](asra_formula_images/environment_tuple.png)

Plain-text fallback: `E = (S, A, T, O)`
...
![Observed transition](asra_formula_images/transition_observation.png)

Plain-text fallback: `s_t + a_t -> s_{t+1}`
...
![Action semantics inference](asra_formula_images/action_semantics.png)

Plain-text fallback: `phi_hat(a_t) = argmax_phi P(s_{t+1} | s_t, phi(a_t), theta)`
...
![Argmax phi](asra_formula_images/argmax_phi.png)

Plain-text fallback: `argmax_phi`
...
![Inferred action meaning](asra_formula_images/move_up.png)

Plain-text fallback: `phi_hat(ACTION2) = MOVE_UP`
...
![Information-theoretic action selection](asra_formula_images/info_gain_action.png)

Plain-text fallback: `a_t* = argmax_{a_t} I(H_t ; s_{t+1} | a_t)`
...
![World model transition](asra_formula_images/world_model.png)

Plain-text fallback: `M_t: (s_t, a_t, h_t) -> s_hat_{t+1}`
...
![Entropy reduction action selection](asra_formula_images/entropy_reduction.png)

Plain-text fallback: `a* = argmax_a [ H(M_t) - H(M_{t+1} | a) ]`
...
![Biological dynamics model](asra_formula_images/bio_dynamics.png)

Plain-text fallback: `x_{t+1} = F(x_t, p_t, omega_t)`
...
![Biological transition](asra_formula_images/bio_transition.png)

Plain-text fallback: `cell state + perturbation -> next cell state`
