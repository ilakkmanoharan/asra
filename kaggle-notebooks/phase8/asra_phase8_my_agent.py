"""ASRA Phase 8 agent for ARC Prize 2026 — ARC-AGI-3 (Kaggle Swarm).

Phase 2 object scenes + Phase 3 exploration memory + Phase 4 causal semantics +
Phase 8 goal inference: hypothesis generation, progress detection, experiment planning.
"""
...
# --- Phase 8 goal inference (embedded) ---
...
class GoalHypothesisEngine:
    """Phase 8 embedded: goal templates, progress signals, hypothesis ranking, experiment hints."""
...
if __name__ == "__main__" and len(sys.argv) > 1 and sys.argv[1] == "--self-test":
    ...
    print("runtime self-test OK (Phase 8)")
    sys.exit(0)
...
class ASRAExplorer:
    """Phase 2 object hints + Phase 3 exploration + Phase 4 causality + Phase 8 goals."""
...
class ASRAAgent(Agent):
    """Phase 8: object scenes + exploration + causality + goal hypothesis hints."""
    ...
    def _to_game_action(
        ...
        ga.reasoning = (
            f"ASRA Phase8: {action_name} | objects={n_obj} "
...
def run_swarm() -> Any:
    ...
    print(f"ASRA Phase8 Swarm: {len(games)} games | object+exploration+causality+goal hints")
    swarm = Swarm(
        "asra",
        os.environ.get("ARC_BASE_URL", "https://three.arcprize.org"),
        games,
        tags=["asra-v0.9-phase8"],
    )
    return swarm.main()
