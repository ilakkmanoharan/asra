"""ASRA Phase 3 agent for ARC Prize 2026 — ARC-AGI-3 (Kaggle Swarm).

Phase 2 compact object-scene hints + Phase 3 visit memory, novelty/usefulness scoring,
and loop-avoidance for exploration (Navigation & Memory Engine v1).
"""
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--self-test":
        ...
        print("runtime self-test OK (Phase 3)")
        sys.exit(0)
...
EXPLORATION_HINT_WEIGHT = float(os.environ.get("ASRA_EXPLORATION_HINT_WEIGHT", "0.45"))
...
class ActionSemanticsInferencer:
    ...
    def infer(self, state_hash_value: str, action: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "mean_delta_objects": float(np.mean(obj_deltas)) if obj_deltas else 0.0,
        }


class ASRAExplorer:
    """Phase 2 object hints + Phase 3 visit/novelty exploration hints."""
...
class ASRAAgent(Agent):
    """Phase 3: object scenes + exploration memory/novelty hints."""
    ...
    def _to_game_action(self, action_name: str, grid: Any, scene_hint: Optional[Dict[str, Any]] = None) -> GameAction:
        ...
        ga.reasoning = (
            f"ASRA Phase3: {action_name} | objects={n_obj} "
...
def run_swarm() -> Any:
    ...
    print(f"ASRA Phase3 Swarm: {len(games)} games | object_hints=on exploration_hints=on")
    swarm = Swarm(
        "asra",
        os.environ.get("ARC_BASE_URL", "https://three.arcprize.org"),
        games,
        tags=["asra-v0.5-phase3"],
    )
    return swarm.main()
