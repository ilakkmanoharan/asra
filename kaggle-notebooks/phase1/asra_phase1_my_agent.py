"""ASRA Phase 1 agent for ARC Prize 2026 — ARC-AGI-3 (Kaggle Swarm)."""
...
def _venv_dir(working: str) -> str:
    if os.environ.get("ASRA_VENV"):
        return os.environ["ASRA_VENV"]
...
def write_submission_parquet(path: str, scorecard: Optional[Any] = None) -> None:
    ...
    cols = ["game_id", "score", "levels_completed", "actions", "completed"]
    pd.DataFrame(rows)[cols].to_parquet(path, index=False)
