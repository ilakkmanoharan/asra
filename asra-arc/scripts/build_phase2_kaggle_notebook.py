#!/usr/bin/env python3
"""Build Kaggle Phase 2 submission notebook from asra_phase2_my_agent.py."""
...
def main() -> None:
    ...
    agent_path = repo / "kaggle-notebooks" / "asra_phase2_my_agent.py"
    out_path = repo / "kaggle-notebooks" / "asra-phase-2-arc-prize-2026.ipynb"
...
if result.returncode != 0:
    print(result.stderr)
    raise RuntimeError('my_agent.py Phase 2 self-test failed')
print('Phase 2 my_agent self-test OK')'''
        ...
        "cells": [
            {
                ...
                "metadata": {},
                "source": [
                    "# ASRA Phase 2 — Kaggle submission (ARC Prize 2026)\n\n",
                    "Phase 2 adds **compact object-scene hints** to exploration and transition logging. ",
                    "Uses dedicated `asra_venv` bootstrap (same as Phase 1 v4-fixed).\n\n",
                    "**Submit:** Save & Run All → Submit to competition. Scoring re-runs `my_agent.py`.\n\n",
                    "Agent tag: `asra-v0.4-phase2`",
                ],
            },
            {"cell_type": "markdown", "metadata": {}, "source": ["## 0. Bootstrap venv + mirror competition assets"]},
            {"cell_type": "code", "metadata": {}, "source": [bootstrap_cell], "outputs": [], "execution_count": None},
            {"cell_type": "markdown", "metadata": {}, "source": ["## 1. Write `my_agent.py` (Phase 2)"]},
